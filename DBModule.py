...
def LoadHashTable(textFile):
    """Load a hash table from a given file, handling empty or missing files."""
    file_path = os.path.join(os.getcwd(), str(textFile) + ".txt")

    if not os.path.exists(file_path):
        print(f"⚠️ Warning: {textFile}.txt does not exist. Creating a new one.")
        return {}

    try:
        with open(file_path, "rb") as savefile:
            data = pickle.load(savefile)
            return data if data else {}
    except (EOFError, pickle.UnpicklingError):
        print(f"⚠️ Warning: {textFile}.txt is empty or corrupted. Resetting.")
        return {}
